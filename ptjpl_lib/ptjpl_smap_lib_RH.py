...
def ptjpl_smap(lat,
               ...
               fAPARmax,
               net_radiation,
               rh_smooth, # < new for SMAP ET
               WP,    # < new for SMAP ET
               FC,    # < new for SMAP ET
               CH,    # < new for SMAP ET
               SM,    # < new for SMAP ET
               QC_AM, # < new for SMAP ET
               QC_PM, # < new for SMAP ET
               verbose=True,
    ...
    vapor_pressure_deficit_15_day = saturation_vapor_pressure_15_day - water_vapor_moving_window_kPa
    ...
    relative_surface_wetness[air_temperature_mean<-1.]=0.
    ...
    #soil_evaporation = np.where(np.isnan(soil_evaporation), np.nan, soil_evaporation)
    soil_evaporation[air_temperature_mean<-1.]=0.
    ...
    soil_evaporation_rew[soil_evaporation_rew < 0.] = np.nan
    soil_evaporation_rew[air_temperature_mean<-1.] = 0.
    ...
    canopy_transpiration[canopy_transpiration < 0] = np.nan
    ...
    def weighting_function(rh,vwc):
        return rh**(4*((1.0-vwc))*(1.-(rh)))

    FM_weight = weighting_function(rh_smooth,SM)
    # FM_weight = weighting_function(relative_humidity_15_day,SM)

    REWTCHPET_weight = 1.0-FM_weight
    # AA['WEIGHT_PM'] =  weighting_function_2(AA.RH_roll.rolling(window=20,min_periods=5, center = False).mean(),SM_RZ)
    # AA['WEIGHT_SM'] = 1.0-AA.WEIGHT_PM
    
    rh_trew = REWTCHPET_weight * rew_constraint_T_CH_PET + FM_weight * plant_moisture_constraint 
    #< changes to the code for

    # ************* Revisions for RS2 *************
    canopy_transpiration_rew = PRIESTLEY_TAYLOR_ALPHA * (1. - relative_surface_wetness) * green_canopy_fraction * plant_temperature_constraint * rh_trew * epsilon * canopy_net_radiation
    canopy_transpiration_rew[canopy_transpiration_rew < -0.] = np.nan

    # ORIGINAL, includes SQRT and FM
    ...
    # NO FM NO SQRT
    # canopy_transpiration_rew = PRIESTLEY_TAYLOR_ALPHA * (1. - relative_surface_wetness) * green_canopy_fraction * plant_temperature_constraint * rew_constraint_T_CH_PET* epsilon * canopy_net_radiation
    # canopy_transpiration_rew[canopy_transpiration_rew < -0.] = np.nan
    # FOR AGU CARBON WATER ANALYSIS 
    #canopy_transpiration_o = PRIESTLEY_TAYLOR_ALPHA * green_canopy_fraction * plant_temperature_constraint *plant_moisture_constraint * epsilon *canopy_net_radiation
    #canopy_transpiration_o_fT_fM = PRIESTLEY_TAYLOR_ALPHA * plant_temperature_constraint *plant_moisture_constraint * epsilon *canopy_net_radiation
    #canopy_transpiration_o_fM_fG = PRIESTLEY_TAYLOR_ALPHA * green_canopy_fraction * plant_moisture_constraint * epsilon *canopy_net_radiation
    #canopy_transpiration_o_fT_fG = PRIESTLEY_TAYLOR_ALPHA * green_canopy_fraction * plant_temperature_constraint * epsilon *canopy_net_radiation
    #canopy_transpiration_fG  = PRIESTLEY_TAYLOR_ALPHA * green_canopy_fraction * epsilon * canopy_net_radiation
    #canopy_transpiration_fT  = PRIESTLEY_TAYLOR_ALPHA * plant_temperature_constraint * epsilon * canopy_net_radiation
    #canopy_transpiration_fM  = PRIESTLEY_TAYLOR_ALPHA * plant_moisture_constraint * epsilon * canopy_net_radiation
    #canopy_transpiration_n_ftrew_ft = PRIESTLEY_TAYLOR_ALPHA * plant_temperature_constraint * rew_constraint_T_CH_PET* epsilon * canopy_net_radiation
    #canopy_transpiration_n_ftrew_fg = PRIESTLEY_TAYLOR_ALPHA * green_canopy_fraction * rew_constraint_T_CH_PET* epsilon * canopy_net_radiation
    #canopy_transpiration_n = PRIESTLEY_TAYLOR_ALPHA * green_canopy_fraction * plant_temperature_constraint * rew_constraint_T_CH_PET* epsilon * canopy_net_radiation
    #canopy_transpiration_n_ftrew = PRIESTLEY_TAYLOR_ALPHA * rew_constraint_T_CH_PET* epsilon * canopy_net_radiation
    ...
    interception_evaporation[interception_evaporation < 0] = np.nan
    ...
    potential_evapotranspiration[mask]=np.nan
    potential_transpiration[mask]=np.nan
